...
description: 全自动AI热点文章发布技能。专门用于追踪Seedance 2.0及相关AI产品的最新动态，撰写高质量英文文章（默认约700词），生成配图，并自动保存到网站草稿箱。每周发布2篇精品文章。当用户要求自动发布文章、追踪AI热点、创建Seedance相关内容、或需要自动写作配图发布时使用此技能。
---
...
- **发布状态**: 始终保存为草稿，不直接发布
- **文章语言**: 全英文
- **文章长度**: **约 700 英文词**（默认标准，允许 ±50 词漂移；确有深度选题需更长正文时上限不超过 1500 词，并在选题记录中注明理由）
- **配图数量**: 按字数阶梯自动匹配（约 700 词落在 500–750 档 → **3 张**：封面 + 2 正文；751–1000 → 4 张；1001–1500 → 5 张）
- **封面图**: 第1张图，16:9 比例，简洁风格
- **配图比例**: 4:3 或 16:9

### 定制硬性规则（必须遵守）

以下为用户在本项目后续追加的约定，**优先级高于一般性描述**；执行本技能时须默认满足。

1. **正文长度锚点**  
   - 默认 **约 700 英文词**，允许 **±50 词** 漂移。  
   - 确需更长正文时：**总长不超过 1500 词**，且在选题/任务记录中写明 **加长理由**（如深度对比、政策梳理）。  
   - 脚本参考：`scripts/run_two_seedance_drafts.py` 中 `TARGET_WORD_COUNT = 700` 为字数锚点；新写文章在定稿前应用工具或脚本统计词数，偏离默认区间须有意为之。

2. **配图数量与字数绑定**  
   - 严格按上表阶梯：**500–750 词（含默认 ~700）→ 3 张**；751–1000 → 4 张；1001–1500 → 5 张。  
   - **不得**为省额度在未达到下一阶梯时擅自减图；若因 API 配额等原因缺图，须在保存草稿前向用户说明，并仅在正文中嵌入已成功上传 R2 的图片。

3. **标题去重（CMS 全库）**  
   - **撰写或定稿前**必须拉取站点已有标题并去重：  
     `GET https://www.seedance-2.info/api/articles?scope=all`  
     请求头：`Authorization: Bearer` 与 `draft_saver.py` 中 `BEARER_TOKEN` 一致（须能读取 `scope=all`；若你方环境改为仅写权限密钥可读全量，则以实际 token 为准）。  
   - 将返回列表中每条 `title` **归一化**后比较：去首尾空格、合并连续空白、统一大小写（或按站点惯例统一 title case）。  
   - 新稿 `title` **不得与任一已有标题完全相同**；若与现有标题仅差虚词、语序微调或高度同质，须 **改写角度或副标题**，并在选题记录中一句话说明与已有文章的差异。  
   - 同一用户短期内重复请求「两篇新文章」时，须重新执行本去重步骤，**禁止**直接复用已上传过的固定标题模板（除非用户明确要求修订旧稿）。

4. **流程不缩水**  
   - 出稿、配图、**R2 上传**、组装 `bodyHtml`、`POST` **草稿** 的链路保持不变；禁止用未上传 R2 的临时图床 URL 冒充终稿嵌入正文。
...
- **标题不冲突**: 已通过 `GET /api/articles?scope=all` 与全库 `title` 去重（归一化后无完全相同标题；高度同质须换角度并留记录）——见上文「定制硬性规则 → 标题去重」
...
**必须遵守:**
1. **全英文写作** - 语法正确，表达地道
2. **约 700 词（默认）** - 允许 ±50 词；深度稿不超过 1500 词
...
- [ ] 句子多样化
- [ ] 关键词自然分布
- [ ] 字数接近默认目标（约 700 词 ±50），或已注明加长理由且在 1500 词以内
...
2.5 [标题去重] → `GET /api/articles?scope=all` 拉取已有 title，归一化比对；新选题标题不得与全库重复，同质须改角度
